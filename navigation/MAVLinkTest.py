...
class mission_item:
    def __init__(self, i, current, x, y, z):
        self.seq = i
        self.frame = mavutil.mavlink_FRAME_GLOBAL_RELATIVE_ALT # Yse Global Latitude and Longitude for position data
...
def arm(the_connection):
    print("-- Arming")
    the_connection.mav_command_long_send(the_connection.target_system, the_connection.target_component,
                    mavutil.mavlink.MAV_CMD_NAV_WAYPOINT_ARM_DISARM, 0, 1, 0, 0, 0 ,0 ,0 ,0)
...
def upload_mission(the_connection, mission_items):
    ...
    the_connection.mav.mission_count_end(the_connection.target_system, the_connection.target_component, n, 0)
...
def set_return(the_connection):
    ...
    the_connection.mav.command_long_send(the_connection.target_system, the_connection.target_component,
                                         mavutil.mavlink.MAV.CMD.NAV.RETURN.TO.LAUNCH, 0, 0, 0, 0, 0 ,0 ,0 ,0)
...
def start_mission(the_connection):
    print("-- Mission Start")
    the_connection.mav.command_long_send(the_connection.target_system, the_connection.target_component,
                                         mavutil.mavlink.MAV.CMD.MISSION_START, 0, 0, 0, 0, 0 ,0 ,0 ,0)
    ack(the_connection, "COMMAND_ACK")
